...
uint32_t tp_calx = 18808832; //   11470659;                        //7472920;
uint32_t tp_caly =  18810615; //    8456015;                   //    122224794;  // 122617985 (yb en yt verwisseld)
...
{
#if USE_TOUCH
	//gpio_intr_disable(touch_interrupt_pin);
	int result = -1;
    int32_t X=0, Y=0, tmp;
...
*/
	///21-11-2017
	int xleft = 6; // 8; //179; //52; //178; // 131; ( -20, -50)
	int xright = 2009; //1947; //1946; //1865;  //-50,50
	int ytop = 149; //287; //288;  //1763 ; //1875;
	int ybottom = 1767; //1783; //1757; // 175; +200

	/*
	int xleft = 8; //178; // 131; ( -20, -50)
	int xright = 1947; //1865;  //-50,50
	int ytop = 287; //175;  //1763 ; //1875;  domi 288
	int ybottom = 1783; //1757; // 175; +200
	*/

	//printf("cal xl %d",xleft);printf(" xr %d \n", xright);
	//printf("cal yb %d", ybottom);printf(" yt %d \n", ytop);
	int width = _width;   // deze waarden worden gwijzigd bij P, PF,L,LF
	int height = _height;

	int tus = X;
	X=Y; Y = tus;
//printf("xraw %d  yraw %d \n", X,Y);
//printf("xraw* %d  yraw* %d \n", 1000*(X-xleft)/(xright-xleft),1000*(ybottom-Y)/(ybottom-ytop));

//  opm; afhankelijk van de orientation wisselne de height en width waarden.  Dus eigenlijk zouden de twee bolgende if niet nodig ijn. Maar dat zijn ze wel.

if (((xright - xleft) != 0) && ((ybottom - ytop) != 0)) {
		if (orientation==LANDSCAPE || orientation==LANDSCAPE_FLIP)	{
		   // omzetten naar ratio
				//X = ((X- xleft) * width) / (xright - xleft);
		     	//Y = ((ybottom-Y) * height) / (ybottom - ytop);
			X = ((X-xleft ) * width) / (xright - xleft);
			Y = ((ybottom-Y) * height) / (ybottom - ytop);
		   // clip tegen de randen
		     //	if (X < 0) X = 0;
	   ...
	   if (orientation==PORTRAIT || orientation == PORTRAIT_FLIP)	{
		   // omzetten naar ratio
				...
				Y = ((ybottom-Y) * width) / (ybottom - ytop);
           // clip tegen de randen
				if (X < 0) X = 0;
	...
	printf("cor x %d", X);printf(" y %d \n", Y);
	printf("heigth%d  width %d \n", height, width);

	switch (orientation) {
		case LANDSCAPE:
			 printf("view L x %d", X);printf(" y %d \n", Y);
				break;
		case PORTRAIT:
			tmp = X;
				X =width - Y - 1;
		...
		case PORTRAIT_FLIP:
			...
			X = Y;
			Y = height - tmp - 1;
			printf("view PF x %d", X);printf(" y %d \n", Y);
			break;
		case LANDSCAPE_FLIP:
			X = width - X - 1;
			Y = height - Y - 1;
			printf("view LF x %d", X);printf(" y %d \n", Y);
			break;
	}
/*
	int XC=X; int YC = Y;
	 ...
	 tmp = XC;
	 X = width - YC - 1;
	 Y = tmp; printf("view P x %d", X);printf(" y %d \n", Y);
...
exit:
//gpio_intr_enable(touch_interrupt_pin);
