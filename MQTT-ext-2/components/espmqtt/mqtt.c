...
void deliver_publish(mqtt_client *client, uint8_t *message, int length)
{
   ...
   // vTaskDelay(2000 / portTICK_RATE_MS);
    do
    {
       ...
       // mqtt_info("Data receiveded: %d/%d bytes ", mqtt_len, total_mqtt_len);  // mei 2017 jan
       // mqtt_info("Topic receiveded: %s ", event_data.topic);  // mei 2017 jan
        //mqtt_info("Data receiveded: %s ", event_data.data);  // mei 2017 jan
...
void test ( mqtt_client *xx, char *txt){
mqtt_publish(xx, "/test/jan", txt, 2, 2, 0);
}
